...
<script>
	window.onload = function(){
		const btnPassChange = document.getElement('btnPassChange');
		const formFindPassChange = document.getElement('formFindPassChange');
		
		btnPassChange.onclick = function(e){
			e.preventDefault();
			
			if(isPassOk){
				formFinPassChange.submit();
			}else{
...
<main id="user">
    <section class="find findPassChange">
        ...
        <div>
            ...
            <a href="/Jboard2/user/login.do" id="btnPassChange" class="btn btnNext">비밀번호 변경</a>
        </div>
    </section>
</main>
